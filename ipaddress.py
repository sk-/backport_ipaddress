...
def isinstance(x, types):
    """Overload of isinstance so to consider int==long."""
    if type(types) is tuple:
        if int in types:
            types += (long,)
    elif types in (int, long):
        types = (int, long)

    return __builtin__.isinstance(x, types)
...
def int_to_bytes(val, length=None, byteorder="big", signed=False):
    """
    ...
    length = int(math.ceil(bit_length(val) / 8.0)) if length is None else length
    fill_char = "0"
    ...
    if abs(bit_length(val)) > (length * 8):
...
def int_from_bytes(packed, byteorder="big", signed=False):
    """
...
def v4_int_to_packed(address):
    ...
    """
    try:
        return int_to_bytes(address, 4, 'big')
    except Exception:
...
def v6_int_to_packed(address):
    ...
    """
    try:
        return int_to_bytes(address, 16, 'big')
    except Exception:
...
def _count_righthand_zero_bits(number, bits):
    ...
    if number == 0:
        return bits
    return min(bits, bit_length(~number & (number-1)))
...
def summarize_address_range(first, last):
    ...
    while first_int <= last_int:
        nbits = min(_count_righthand_zero_bits(first_int, ip_bits),
                    bit_length(last_int - first_int + 1) - 1)
...
class _IPAddressBase(_TotalOrderingMixin):
    ...
    def _prefix_from_ip_int(cls, ip_int):
        ...
        if leading_ones != all_ones:
            ...
            details = int_to_bytes(ip_int, byteslen, 'big')
...
class _BaseV4(object):
    ...
    def _ip_int_from_string(cls, ip_str):
        ...
        try:
            return int_from_bytes(map(cls._parse_octet, octets), 'big')
    ...
    def _string_from_ip_int(cls, ip_int):
        ...
        """
        return '.'.join(map(str, int_to_bytes(ip_int, 4, 'big')))
...
class IPv4Address(_BaseV4, _BaseAddress):
    ...
    def __init__(self, address):
        ...
        if isinstance(address, bytearray):
            ...
            self._ip = int_from_bytes(address, 'big')
            return
...
class IPv4Interface(IPv4Address):
    ...
    def __init__(self, address):
        if isinstance(address, (bytearray, int)):
...
class IPv4Network(_BaseV4, _BaseNetwork):
    ...
    def __init__(self, address, strict=True):
        ...
        if isinstance(address, (int, bytearray)):
